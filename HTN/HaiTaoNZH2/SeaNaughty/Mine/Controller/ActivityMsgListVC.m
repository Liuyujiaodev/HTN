...
@interface ActivityMsgListVC () <UITableViewDelegate, UITableViewDataSource,WKNavigationDelegate>
...
@implementation ActivityMsgListVC
...
- (void)viewDidLoad {
    ...
    [_param setObject:@"2" forKey:@"type"];
...
{
    ActivityMsgCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ActivityMsgCell"];
    MsgModel *model = _dataArray[indexPath.section];
...
- (void)getPushList
{
    ...
    MBProgressHUD *loadingHud = [MBProgressHUD showHUDAddedTo:[UIApplication sharedApplication].delegate.window.rootViewController.view animated:YES];
        ...
        if ([[result[@"code"] stringValue] isEqualToString:@"0"])
        {
            ...
            if (modelList.rows.count > 0)
            {
                for (MsgModel *model in modelList.rows)
                {
                    ...
                    model.subTitleHight = [model.subtitle boundingRectWithSize:CGSizeMake(G_SCREEN_WIDTH-50, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]} context:nil].size.height;
                    model.titleHight = [model.alert boundingRectWithSize:CGSizeMake(G_SCREEN_WIDTH-50, 1000) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]} context:nil].size.height;
                    ...
                    if (!model.endTime || [[NSString stringWithFormat:@"%@", model.endTime] isEqualToString:@""])
                    {
                        model.isEnd = NO;
                    }
