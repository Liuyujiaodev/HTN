...
- (void)setUpWithItems:(NSArray<NSString *> *)items childVCs:(NSArray<UIViewController *> *)childVCs{
    ...
    self.contentView.contentSize = CGSizeMake(items.count * self.view.ll_width, 0);
...
- (void)showChildVCViewAtIndex:(NSInteger)index{
    ...
    vc.view.frame = CGRectMake(index * self.contentView.ll_width, 0, self.contentView.ll_width, self.contentView.ll_height);
    [self.contentView addSubview:vc.view];
    
    // 滑动到对应位置
    [self.contentView setContentOffset:CGPointMake(index * self.contentView.ll_width, 0) animated:YES];
...
- (void)viewWillLayoutSubviews{
    ...
    if (self.segmentBar.superview == self.view) {
        ...
        self.segmentBar.frame = CGRectMake(0, 60, self.contentView.ll_width, 35);
        
        CGFloat contentViewY = self.segmentBar.ll_y + self.segmentBar.ll_height;
        CGRect contentFrame = CGRectMake(0, contentViewY, self.view.ll_width, self.view.ll_height - contentViewY);
        self.contentView.frame = contentFrame;
        self.contentView.contentSize = CGSizeMake(self.childViewControllers.count * self.view.ll_width, 0);
        
        return;
    }
    
    CGRect contentFrame = CGRectMake(0, 0, self.view.ll_width, self.view.ll_height);
    self.contentView.frame = contentFrame;
    self.contentView.contentSize = CGSizeMake(self.childViewControllers.count * self.view.ll_width, 0);
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    NSInteger index = self.contentView.contentOffset.x/self.contentView.ll_width;
