...
- (void)layoutSubviews
{
    ...
    _textField.frame = CGRectMake(0.5*_height, 0, _height, _height);
    _increaseBtn.frame = CGRectMake(1.5*_height, 0, 0.5 * _height, _height);
    
    if (_decreaseHide && _textField.text.integerValue < _minValue) {
        _decreaseBtn.frame = CGRectMake(_width-_height, 0, 0.5 *_height, _height);
    } else {
        _decreaseBtn.frame = CGRectMake(0, 0, 0.5 *_height, _height);
    }
}
