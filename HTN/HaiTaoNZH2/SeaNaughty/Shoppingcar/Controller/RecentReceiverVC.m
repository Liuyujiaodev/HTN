...
@interface RecentReceiverVC () <UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate, RecentReceiverVCDelegate>
...
@implementation RecentReceiverVC
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_dataArray.count>0 || _searchArray.count>0)
    {
        ...
        NSString *strPhone = model.receivePhone ? model.receivePhone : @"";
        NSString *strId = model.receiveIdCard ? model.receiveIdCard : @"";
        
        NSString *newString = [NSString stringWithFormat:@"%@    %@    %@", model.receiveName, strPhone, strId];
...
- (void)orderAction
{
    if (_postModel)
    {
        if (_delegate && [_delegate respondsToSelector:@selector(postReceriverModel:)])
        {
            [_delegate postReceriverModel:_postModel];
            [self.navigationController popViewControllerAnimated:YES];
        }
    }
