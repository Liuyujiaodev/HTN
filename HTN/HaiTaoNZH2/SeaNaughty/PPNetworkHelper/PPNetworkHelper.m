...
+ (NSURLSessionTask *)uploadImagesWithURL:(NSString *)URL
    ...
    NSURLSessionTask *sessionTask = [_sessionManager POST:URL parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (NSUInteger i = 0; i < images.count; i++) {
            ...
            NSString *namee = [NSString stringWithFormat:@"file%lu",(unsigned long)i];
          ...
          block:(BlockType)block
{
    ...
    [manager POST:urlString parameters:params constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        for (int i = 0; i < imageArray.count; i++)
        {
            ...
            formatter.dateFormat = @"yyyyMMddHHmmss";
            NSString *str = [formatter stringFromDate:[NSDate date]];
            NSString *fileName = [NSString stringWithFormat:@"%@%i.jpeg", str, i];
            NSString *strFile = [NSString stringWithFormat:@"file%i", i];
            NSLog(@"%@======%@", fileName, strFile);
            [formData appendPartWithFileData:data name:strFile fileName:fileName mimeType:@"image/jpeg"];
        }
...
+ (void)initialize {
    ...
    // 打开状态栏的等待菊花
//    _sessionMana
    [_sessionManager.requestSerializer setValue:@"$2a$08$bW3CufG4d5HGlBRWkgRSyuc5E0viJ40V8qQdDOx9TzAy84OMipXVa" forHTTPHeaderField:@"token"];
